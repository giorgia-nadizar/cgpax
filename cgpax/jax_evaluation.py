...
def __evaluate_program__(program: Callable, program_state_size: int, rnd_key: random.PRNGKey,
    ...
    def rollout_loop(
        ...
        rew_forward += new_state.metrics.get("reward_forward",
                                             new_state.metrics.get("reward_run", 0)) * new_active_episode
        rew_ctrl += new_state.metrics.get("reward_ctrl", 0) * new_active_episode
        rew_healthy += new_state.metrics.get("reward_healthy",
                                             new_state.metrics.get("reward_survive", 0)) * new_active_episode
        new_rew_carry = rew_forward, rew_ctrl, rew_healthy
    ...
    (final_env_state, _, cum_reward, (rs_forward, rs_ctrl, rs_healthy), _), _ = lax.scan(
        f=rollout_loop,
