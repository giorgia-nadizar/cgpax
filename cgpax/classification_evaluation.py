...
def _evaluate_classification_program(program: Callable, program_state_size: int, x_values: jnp.ndarray,
    ...
    predicted_classes = vmap(_compute_prediction)(x_values)
    errors = predicted_classes != y_values
...
def evaluate_cgp_genome(genome: jnp.ndarray, config: Dict, x_values: jnp.ndarray, y_values: jnp.ndarray,
                        inner_evaluator: Callable = _evaluate_classification_program,
