...
def count_operations(formula: str) -> int:
    operations = 0
    for symbol in [x.symbol for x in function_set_numeric.values()]:
...
def count_non_arithmetic_operations(formula: str) -> int:
    operations = 0
    non_arithmetic_symbols = [x.symbol for x in function_set_numeric.values() if x.symbol not in arithmetic_symbols]
