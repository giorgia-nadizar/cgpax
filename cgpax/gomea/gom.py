    ...
    for f_idx in range(len(array_fos)):
        ...
        if not diversity_preservation:
            ...
            def _diversity_insertion(g_idx: int, carry: Tuple[jnp.ndarray, int]):
                ...
                new_n_uniques = jnp.unique(tmp_genotypes.astype(jnp.int32), axis=0, size=genotypes.shape[0]).shape[0]
            ...
            diversity_preservation_mask, n_uniques = lax.fori_loop(0, len(fitnesses), _diversity_insertion,
                                                                   (jnp.zeros_like(better_offspring, dtype=int),
                                                                    len(jnp.unique(genotypes.astype(int), axis=0))))
            unique_ratio = n_uniques / len(genotypes)
            genotypes = jnp.where(jnp.logical_and(better_offspring, diversity_preservation_mask)[:, None],
                                  offspring_genotypes, genotypes)
